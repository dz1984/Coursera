...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def descendingByRetweet: TweetList = {
    def loops(sets: TweetSet, lists: TweetList): TweetList = {
      // TODO: check
      if (!sets.contains(elem)) lists
      else if (lists.isEmpty) new Cons(elem,lists)
      else if (elem.retweets > lists.head.retweets) loops(sets.remove(elem), new Cons(elem, lists))
      else new Cons(lists.head, loops(sets, lists.tail))
    }
    loops(this, Nil)
  }
}
